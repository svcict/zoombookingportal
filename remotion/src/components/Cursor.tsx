...
// Simulated pointer: interpolates through a scene-local list of {frame,x,y}
// keyframes, and renders click ripples at the given click frames.
  ...
  return (
    <>
      {clicks.map((clickFrame) => {
        ...
        const scale = interpolate(local, [0, 24], [0.2, 2.6], { easing: Easing.out(Easing.cubic) });
        const opacity = interpolate(local, [0, 24], [0.65, 0], { extrapolateRight: 'clamp' });
        return (
          <div
            key={clickFrame}
            style={{
